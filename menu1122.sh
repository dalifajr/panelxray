...
# // USERNAME IZIN IPP
rm -f /usr/bin/user
username=$(cat /usr/bin/user 2>/dev/null || hostname -s 2>/dev/null || echo User)
echo "$username" >/usr/bin/user

# // VALIDITY
rm -f /usr/bin/e
valid=$(cat /usr/bin/e 2>/dev/null || echo Lifetime)
...
Exp1=$(cat /usr/bin/e 2>/dev/null || echo Lifetime)
if [[ $today < $Exp1 ]]; then
sts="${Info}"
else
sts="${Error}"
...
clear
ssh_service=$(systemctl status ssh 2>/dev/null | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
if [[ -z "$ssh_service" ]]; then
    ssh_service=$(systemctl status sshd 2>/dev/null | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
fi
dropbear_service=$(systemctl status dropbear 2>/dev/null | grep Active | awk '{print $3}' | cut -d "(" -f2 | cut -d ")" -f1)
...
REVISI="https://raw.githubusercontent.com/dalifajr/panelxray/main/"
